  ...
  module Storage
    class FileSystem < Storage
      ...
      def bucket_exist(bucket = "root")
        Dir.entries(contents).include?(bucket) && File.directory?(File.join(contents, bucket))
      end
      ...
      def get(bucket, key)
        if !File.directory?(contents(bucket, key)) # and bucket_files(bucket).include?(key)
      ...
      def put(bucket, key, value, opts = {})
        ...
        meta = Blobsterix.wait_for(proc do
        ...
        Blobsterix.wait_for(proc { Blobsterix.cache.invalidate(Blobsterix::BlobAccess.new(:bucket => bucket, :id => key)) })
      ...
      def list_buckets
        BucketList.new do |l|
          Dir.entries("#{contents}").each do|dir|
            l.buckets << Bucket.new(dir, time_string_of(dir)) if File.directory?(File.join("#{contents}", dir)) && !(dir == '.' || dir == '..')
          end
        end
      end
      ...
      def collect_bucket_entries(bucket, b, opts)
        ...
        current_obj = Blobsterix::DirectoryList.each_limit(contents(bucket), :limit => 20, :start_path => start_path) do |_path, file|
